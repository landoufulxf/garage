...
class S3SyncThread(threading.Thread):
    ...
    def run(self):
        ...
        command = ("aws s3 sync {remote_dir} {local_dir} "
                   ...
                   "--content-type \"UTF-8\"").format(
                       local_dir=local_dir, remote_dir=remote_dir)
        while True:
            fail = os.system(command)
            if fail:
                warnings.warn(
                    "Problem running the s3 sync command. You might want to "
                    "run ./scripts/sync_s3.py manually in a shell to inspect.")
            if self.stopped():
                break
...
def _launch_workers(exp_key, n_workers, host, port, result_db_name):
    ...
    for i in range(n_workers):
        p = multiprocessing.Process(
            ...
            args=(exp_key, i, host, port, result_db_name))
        jobs.append(p)
        p.start()
        time.sleep(1)
    return jobs

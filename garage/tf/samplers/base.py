...
class BaseSampler(Sampler):
    def __init__(self, algo):
        ...
        input_tensor, dist = self.algo.policy.distribution_factory.create_empty_dist(self.algo.policy.recurrent)
        entropy = tf.reduce_mean(tf.reduce_sum(dist.entropy(), axis=-1))
        self._f_entropy = tensor_utils.compile_function(
            inputs=input_tensor,
            outputs=entropy,
        )
    ...
    def process_samples(self, itr, paths):
        ...
        if not self.algo.policy.recurrent:
            ...
            dist_info_list = [agent_infos[k] for k in self.algo.policy.distribution_factory.dist_info_keys]
            ent = self._f_entropy(*(dist_info_list))
